...
@inline Base.:*(x::Number, partials::Partials) = partials*x
...
if NANSAFE_MODE_ENABLED
    @inline function Base.:*(partials::Partials, x::Number)
...
@inline Base.:*(partials::Partials{0,V}, x::Number) where {V} = Partials{0,promote_type(V,typeof(x))}(tuple())
@inline Base.:*(x::Number, partials::Partials{0,V}) where {V} = Partials{0,promote_type(V,typeof(x))}(tuple())
